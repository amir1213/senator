...
local function run(msg, matches)
  if matches[1]:lower() == 'enator' then -- Put everything you like :)
    local about = _config.about_text
    local name = user_print_name(msg.from)
    savelog(msg.to.id, name.." ["..msg.from.id.."] used /teleseed ")
    return about
  end 
  if matches[1]:lower() == "tatslist" then
  ...
  if matches[1]:lower() == "tats" then
    ...
    if matches[2] == "enator" then -- Put everything you like :)
...
return {
  patterns = {
    ...
    "^[Ss](tatslist)$",
    "^[Ss](tats) (group) (%d+)",
    "^[Ss](tats) (senator)",-- Put everything you like :)
		"^[Ss](enator)"-- Put everything you like :)
    }, 
  run = run
}
